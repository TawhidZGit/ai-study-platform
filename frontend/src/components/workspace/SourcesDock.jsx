      ...
      {sources.length === 0 ? (
        ...
        <div className="flex flex-col w-full overflow-y-auto flex-1 no-scrollbar">
          {sources.map((source) => (
            ...
            >
              {/* Icon */}
              <FileText className="h-6 w-6 text-indigo-600 dark:text-indigo-400 group-hover:scale-110 transition-transform duration-200" />
              
              {/* Tiny extension label */}
              <span className="text-[9px] font-bold text-slate-400 dark:text-slate-500 mt-0.5 uppercase opacity-0 group-hover:opacity-100 transition-opacity">
